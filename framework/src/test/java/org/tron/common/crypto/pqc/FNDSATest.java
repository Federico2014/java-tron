...
public class FNDSATest {
  ...
  @Test
  public void slhDsaValidateSignatureRemainsStrictEquality() {
    SLHDSA slhDsa = new SLHDSA();
    slhDsa.validateSignature(new byte[SLHDSA.SIGNATURE_LENGTH]);
    try {
      slhDsa.validateSignature(new byte[SLHDSA.SIGNATURE_LENGTH - 1]);
      fail("SLH-DSA must reject undersized signature");
    } catch (IllegalArgumentException expected) {
      assertTrue(expected.getMessage().contains("signature length"));
    }
    try {
      slhDsa.validateSignature(new byte[SLHDSA.SIGNATURE_LENGTH + 1]);
      fail("SLH-DSA must reject oversized signature");
    } catch (IllegalArgumentException expected) {
      assertTrue(expected.getMessage().contains("signature length"));
    }
  }
  ...
  public void registryIsValidSignatureLengthForFixedSchemesIsStrictEquality() {
    ...
    assertTrue(PqSignatureRegistry.isValidSignatureLength(
        SignatureScheme.SLH_DSA, SLHDSA.SIGNATURE_LENGTH));
    assertFalse(PqSignatureRegistry.isValidSignatureLength(
        SignatureScheme.SLH_DSA, SLHDSA.SIGNATURE_LENGTH - 1));
    assertFalse(PqSignatureRegistry.isValidSignatureLength(
        SignatureScheme.SLH_DSA, SLHDSA.SIGNATURE_LENGTH + 1));
  }
}
